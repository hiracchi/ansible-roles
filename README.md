...
現状、`_unsupported/` 配下のロールはハイフン区切りのままで、上記ルールに違反しています（メンテナンス対象外のため対応は後回しにしています）。それ以外のロールはアンダースコア区切りに統一済みです。新規ロールは必ずアンダースコア区切りにしてください。

### ⚠️ ロール名の一旧リネームについて

2026-08 に、上記の命名規則に従い `_unsupported/` 以外の全ロールをハイフン/ドット区切りからアンダースコア区切りに一旧リネームしました（例: `nfs-client` → `nfs_client`, `system.users` → `system_users`, `proxmox_pci_passthrough` → `proxmox_gpu`）。
